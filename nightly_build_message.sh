...
grep -e ' warning: ' -e 'Warning: ' -e 'WARNING: ' $BUILD_DIR/*.log | grep -v hdv_mainframe_Dict.cc | grep -v 'has modification time' | grep -v 'Clock skew detected' | grep -v 'Obsolete: ' | grep -v 'AMPTOOLS or CLHEP is not defined' >> $REPORT_FILE
